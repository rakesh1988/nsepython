...
def running_status(current_time=None):
    ...
    end_now = datetime.datetime.now(indiaTimeZone).replace(hour=15, minute=30, second=0, microsecond=0)

    # If no current_time is provided, use the actual current time
    if current_time is None:
        current_time = datetime.datetime.now(indiaTimeZone)

    return start_now <= current_time <= end_now
...
def nse_custom_function_secfno(symbols,attributes = ["open", "dayHigh", "dayLow", "previousClose", "lastPrice"]):
    """
    Fetches specific attributes for given symbols, with default attributes.

    :param symbols: List of symbols to filter.
    :param attributes: List of attributes to extract. Defaults to ["open", "dayHigh", "dayLow", "previousClose", "lastPrice"].
    :return: Dictionary with symbol as the key and requested attributes as values.
    """
    positions = nsefetch('https://www.nseindia.com/api/equity-stockIndices?index=SECURITIES%20IN%20F%26O')
    endp = len(positions['data'])
    # Convert symbols to a set for faster membership checks
    symbols_set = set(symbols)
    result = {}
    
    for position in positions['data']:
        symbol = position['symbol']
        if symbol in symbols_set:  # O(1) membership check
            # Dynamically extract requested attributes
            result[symbol] = {attr: position.get(attr, None) for attr in attributes}
    return result
...
def open_interest_spurts():
    payload = nsefetch('https://www.nseindia.com/api/live-analysis-oi-spurts-underlyings')
    print(pd.DataFrame.from_records(payload).to_markdown())
    return payload
